...
public class SampleODEProblemEuler implements SetOfODEs
{
    ...
    public SampleODEProblemEuler(double[] coefficients)
    {
    ...
    public SampleODEProblemEuler(SampleODEProblemEuler source)
    {
    ...
    public boolean equals(Object comparator)
    {
        ...
        if(this.coefficients.length!=(((SampleODEProblemEuler)comparator).coefficients.length)) return false;
        for (int i = 0; i<this.coefficients.length; i++)
            if(this.coefficients[i]!=(((SampleODEProblemEuler)comparator).coefficients[i])) return false;
            return true;
    }
   ...
   //We then return solution, which contains the y[j] values associated with the x[i] values.
    public double[][] solveProblem(double[] x, double delX,double x_0, double[] y_0) {
        ...
        for (int i = 0; i < x.length; i++) {
            ...
            solution[i] = Euler.integrate(x_0, x[i], y_init, delX, 50000, this);
