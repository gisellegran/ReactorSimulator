...
public class CKRK45 {
    ...
    public static double[] integrate(double x_0, double x_f, double[] y_0, double h, int maxIt, double eps, double S, SetOfODEs f) {
        ...
        System.out.println("y4_z\t\ty5_z\t\ty_i+1\t\t"+"e_current\t" + "  " + "e_target\t" + "   " + "e_ratio\t"+ "   " +
                "h\t\t"+ "   " + "x_i+1\t" + "   ");
        while (Math.abs(x_i) < Math.abs(x_f)) {
            ...
            for (int j = 0; j < CKRK45.f; j++) { // f = number of kis
                ...
                for (int l = 0; l < n; l++) { //iterate over each y
                    yi_temp[l] = y_i[l];
                    for (int m = 0; m < j-1; m++) { //iterate over each a
            ...
            //System.out.printf("%5.3f\t%1.3f\t%1.3f\t%1.3f\t%1.3f\t%1.3f\n", y4_z[0], y5_z[0], y_i[0], e_current[0],e_target[0], e_target[0]/e_current[0],h,x_i);
            System.out.printf("%5.3f\t%1.3f\t%1.3f\t%1.3f\t%1.3f\t%1.3f\n", x_i, y_i[0], e_current[0],e_target[0], e_target[0]/e_current[0],h);
